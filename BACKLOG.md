...
- **No enforced `events-index.json` consistency check (gap recorded 2026-09-12, surfaced in review of PR #963).** Nothing compares the IDs in `public/data/events-index.json` against `public/data/events.json`: `validate-partitions.mjs` checks the per-artist partitions only, and the smoke suite asserts the file exists and that the homepage fetches it, not its contents. `public/app.js` and `public/ttc-home.js` both load that index for search, so drift can hide newly added events or keep removed ones listed. The only code that ever made the comparison was `scripts/report-data-stats.mjs`, an uninvoked reporter deleted in PR #963 — it was reachable from no workflow or lane, so the gap is pre-existing rather than newly introduced. Regenerating with `npm run events:partition` after any `events.json` edit is the current mitigation; an enforced check belongs in `validate-partitions.mjs` (note it is itself not in `test:mvp`).
