...
- **Tombstone dedup deletions:** when deleting a row from `events.json` that Ticketmaster still lists, add its ids and/or venue/date to `data/deleted-events.json` in the same change (see `docs/PROVIDER_SYNC.md` and `docs/OPERATIONS.md` → Known incidents). (**Fact added 2026-09-25 by agent:** four Ticketmaster premium-seat upsell rows — "<Artist> | Premium Seats" at venue "AFAS Live Loge", Amsterdam, for five-finger-death-punch, tyla, polyphia and passenger — had been ingested as concert dates and generated a spurious `/venues/afas-live-loge-amsterdam` page. They were removed and tombstoned, and `scripts/sync-ticketmaster-events.py` now withholds a listing whose name carries "| Premium Seats" or whose venue name ends in " Loge" under the existing `travel_package_listing` code.)
...
**Binding status (fact corrected 2026-09-24 by agent).** PR 6a shipped the rules diff: `SAFE_PUBLISHING_RULES.md` now carries path D, and "new artists are never auto-published" applies outside it. `AUTOPROMOTE_ENABLED` is on — the first `auto-promote.yml` run promoted five artists on 2026-09-24 (#1117: hilary-duff, josiah-queen, ha-ash, lukas-graham, passenger) with their dates ingested in the same job. Item 3's "Never auto-publish" still binds every artist outside path D, and the Stage 4 deferral under "Maintenance loop" below stays binding until 6b ships.
...
- Confirm `price-coverage-report.yml` created its `automation:price-coverage` issue, and that the next snapshot-writer summaries show `checks_recorded > 0`. (Fact recorded 2026-09-24 by agent: the workflow has never run — it merged after that day's 09:15 UTC slot, and the agent's manual dispatch was refused with 403 — so its first scheduled run is 2026-09-25 09:15 UTC; a manual dispatch only brings that forward.)
