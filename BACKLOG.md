...
4. **Authorized Ticketmaster/SeatGeek page-price activation:** apply migration `0008_authorized_event_page_pricing.sql`, preview the manual workflow, then run `paired_only=true`, `limit=10`, `apply=true`. Verify 20 fresh exact-event rows with valid price/currency/source URL/current timestamp and no malformed or out-of-catalog rows. Do not enable a schedule until this supervised pass is reviewed. A full-catalog coverage failure is a stop condition, not permission to guess or independently discover pages.
