...
- **Phase 1 proposed scope** (audit §7 items 1–4): prices on city pages; a D1 failure must not print "no snapshot"; record the last check attempt per event and lane so a missing price can show an honest check time; a scheduled price-coverage/staleness report. Not started.
- Relationship to the auto-ingest plan above: not yet ordered against it by the owner.
