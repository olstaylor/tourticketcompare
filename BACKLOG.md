...
**Status (fact recorded 2026-09-21): the code half is merged; what remains is owner configuration, not engineering.** PR #1009 (merged 2026-09-17) added `.github/actions/automation-identity/action.yml`, `scripts/configure-automation-identity.mjs` and a manifest entry, wiring an opt-in App installation token through all ten PR-producing jobs across nine workflows. It is inert until activated. Per `docs/OPERATIONS.md` → "Automation App identity rollout (Milestone 1)", the remaining steps are owner-only: register a private GitHub App owned by `olstaylor` installed on `tourticketcompare` only, store `AUTOMATION_APP_CLIENT_ID` (repository variable) and `AUTOMATION_APP_PRIVATE_KEY` (Actions secret), set `AUTOMATION_APP_ENABLED=true`, then — only after protected-publish evidence is recorded there — activate ruleset `20115269` with no bypass actor. The completion bar above is unchanged: do not mark this complete on configuration alone.
