...
3. **Vivid Seats activation (2026-07-09 update):** the nightly sync automation now exists (`scripts/sync-vividseats-events.mjs` + `.github/workflows/vividseats-cta-sync.yml`, cron shipped commented out) — event-level only. Remaining owner steps: (a) add the `IMPACT_VIVIDSEATS_ACCOUNT_SID`/`IMPACT_VIVIDSEATS_AUTH_TOKEN` GitHub Actions secrets (repo Settings → Actions; same values as the Cloudflare bindings, which are already set); (b) dispatch `mode=preview` to inspect the audit log, then `mode=apply auto_merge=false` for a reviewable first PR and spot-check several Vivid Seats pages before merging; (c) once merged, land a one-line follow-up PR uncommenting the `cron: '30 5 * * *'` schedule. Artist-level Vivid Seats entries remain a separate, still-unscoped follow-up.
