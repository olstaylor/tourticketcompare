...
### 2027 launch-readiness milestone (owner-requested 2026-09-24)

Goal: "safe-to-scale" ahead of the 2027 launch — prices display reliably, pages are rich and indexable where they should be, longform reads as human-written, adding artists needs no babysitting. Five phases, each stopping for owner review: **0 audit → 1 missing prices → 2 indexability → 3 ingestion and self-repair → 4 UX, trust and copy → 5 longform edit**. Work lives on branch `milestone/2027-launch-readiness`; no PR is opened or merged by an agent.

- **Phase 0 — done 2026-09-24, awaiting owner go-ahead.** Findings, measurements and the ranked fix list: [`docs/audits/2026-09-launch-readiness.md`](docs/audits/2026-09-launch-readiness.md). Its §8 lists the decisions the owner owes before Phase 1/2 (schema-offers exception C vs the brief, immutable asset headers, empty-artist indexing, city/venue 404 vs 301).
- **Owner decisions 2026-09-24 (recorded by agent):** keep schema-offers exception C. Direction for the rest of the milestone: favour growth and discoverability over this repo's more restrictive process rules, and propose any rule relaxation explicitly rather than working around it. The brief's own hard limits still hold: no invented data, `/api/out` untouched, no price we don't have.
- **Phase 1 — built on the branch 2026-09-24, awaiting owner review; not merged or deployed.** Audit §7 items 1–4: (1) city pages now query and print snapshots; (2) a failed D1 cache read renders the unchecked state instead of "no snapshot"; (3) both snapshot writers record every completed lookup in D1 `provider_price_checks`, creating the table themselves, and an unpriced card now says which lanes were checked and when, or that the date is not matched on any price-supplying site; (4) `price-coverage-report.yml` (daily, read-only) gates coverage of on-sale mapped dates at ≥ 90% and stale prices at ≤ 25%, and keeps the zero-price-source backlog in the `automation:price-coverage` issue. First live run passed: 98.3% coverage, 216 dates with zero price sources, 0.2% of prices over 12h old.
- **Phase 2 — built on the branch 2026-09-24, awaiting owner review; not merged or deployed.** Owner decisions the same day: pre-on-sale dates show verified resale buttons (`SAFE_PUBLISHING_RULES.md` updated), and growth and discoverability take precedence. What changed:
  - **Artist gate.** An owner-promoted artist needs one tracked date, upcoming or past, to be indexable. beyonce, raye, tate-mcrae and the-weeknd, which have never had a date, go `noindex,follow` and leave the sitemap until their first date lands. Ended tours stay indexed and list their 10 most recent dates as past.
  - **Expired pages.** A city or venue tracked before but with nothing upcoming now 301s (city → `/cities`; venue → its city page, else `/venues`) instead of 404ing.
  - **Duplicates.** A duplicate-H1 check was added to the internal-link audit. It caught the Birmingham UK/US H1 collision, now fixed like the title.
  - **Internal links.** City and venue cards link the artist's indexable artist-city page.
  - **Sitemaps.** Per-type sitemaps behind `/sitemap-index.xml`, which `robots.txt` now advertises; `/sitemap.xml` is unchanged. `events.json` is parsed once per sitemap request.
...
- **Phase 5 — sample of 3 guides edited 2026-09-24 on the stacked branch `milestone/2027-launch-readiness-phase5`; awaiting owner review.** `how-to-compare-concert-ticket-prices`, `how-resale-ticket-pricing-works` and `how-to-avoid-overpaying-for-concert-tickets` were refined, not rewritten: frontmatter (title, H1, description, sources, HowTo), headings and links are unchanged. Notes and `TODO(Ollie)` voice slots: `docs/audits/2026-09-phase5-longform-sample.md`. Nothing further is edited until the owner reviews the sample.
