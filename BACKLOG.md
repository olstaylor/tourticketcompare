...
Last updated: 2026-07-07 (**Item 6 closed — hands-off update automation landed, owner-approved.** New discovery rows now carry `event_name` verbatim from the Ticketmaster Discovery API listing title (sub-item 2 resolved by auto-titling, not suppression); the daily new-show PR auto-merges after its in-run validation suite passes; the nightly data-sync cron is re-enabled with a corrected per-event commit gate and `event_name` in the lossless field set. `tour_name` remains human-gated (#172); the JAY-Z Inglewood "JAY-Z30" owner label is the only open title item. See `PROJECT_STATUS.md` 2026-07-07 header note and `SAFE_PUBLISHING_RULES.md` for the narrow auto-publish exception.)

Prior update: 2026-07-02 (**Affiliate pivot landed** on branch `claude/tour-ticket-compare-affiliates-lgjkp9`: Ticketmaster affiliate machinery removed (plain TM links remain, rendered after affiliate providers), SeatGeek promoted to primary CTA with 16 new artist-level performer-page entries, Vivid Seats fully wired but dormant pending secrets + verified destinations, and batch onboarding tooling (`artists:onboard:propose` / `artists:promote:batch`, ≤20 artists/PR with human spot-check checklist). New operational priorities added below; the "artist-level SeatGeek links" parked item is superseded — SeatGeek price display remains parked.)
...
### 6. Blank `tour_name`/`event_name` recurrence on automation-landed events — **CLOSED 2026-07-07**

**Resolution (owner-directed, minimal-input operation):** the discovery pipeline now populates `event_name` verbatim from the Ticketmaster Discovery API listing title on every new row (`sync-tm-events-write-pr.mjs` / `propose-artists.mjs` → `apply-artists.mjs`), and the nightly sync keeps `event_name` current as a lossless field. Sub-item 2 below was answered by auto-titling rather than defaulting rows to `needs_recheck` — suppression would have added the human step the owner wants to remove, and `event_name` (the rendered title) is a provider-sourced fact. `tour_name` stays human-gated (#172) and blank-`tour_name` remains a warning-level validator item only. Sole open remainder: the JAY-Z Inglewood "JAY-Z30" `tour_name` owner label (also listed in `PROJECT_STATUS.md`). Original record follows.

(found 2026-07-04; titles backfilled 2026-07-06)
...
_(Remaining `#174` Phase B and `#176` deletions are "only if it fits cleanly" judgment calls. Item 6 closed 2026-07-07 — no open engineering priorities remain; active work is operational (item 0) plus routine review of the automation's withheld rows and rolling issues.)_
