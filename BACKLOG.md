...
- **`needs_recheck` re-checks:** 41 events are CTA-suppressed (see `PROJECT_STATUS.md` per-artist table) — re-check periodically for working storefront URLs; never restore from the Discovery `url` field alone.
