...
- **Price comparison:** API/feed snapshots retain their existing workflows. Separately, provider approval text supplied by the owner on 2026-07-13 permits TourTicketCompare to retrieve only the lowest publicly visible price from verified Ticketmaster and SeatGeek event pages when the scoped source is needed. The direct-page lane is manual-only until its supervised sample passes, is capped at one retrieval per canonical provider event ID in any rolling 24 hours, rejects duplicate catalog mappings, stores only the lowest price, currency, exact source URL and retrieval time, and never stores page content. All lanes remain feature-flag, exact-event, approved-source and freshness gated. See `docs/PROVIDER_DATA_POLICY.md`.

## Critical Product Rules

See [SAFE_PUBLISHING_RULES.md](SAFE_PUBLISHING_RULES.md) for the full non-negotiable list. In brief: never invent data, never perform unapproved scraping, never show fake CTAs or price comparisons, never expose credentials client-side, and never modify `/api/out` or affiliate logic without explicit scope. The sole direct-page exception is the authorized Ticketmaster/SeatGeek lowest-price writer, with its exact-event, field-minimization, 24-hour and stop-condition gates.
