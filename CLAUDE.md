...
- **Authorized page-price fallback:** Provider approval text supplied by the owner on 2026-07-13 permits TourTicketCompare to retrieve only the lowest publicly visible price from verified Ticketmaster and SeatGeek event pages when that scoped source is needed. This lane is manual-only until its supervised sample passes, is capped at one retrieval per canonical provider event ID in any rolling 24 hours, rejects duplicate catalog mappings, stores only the lowest price, currency, exact source URL and retrieval time, and never stores page content. It remains feature-flag, exact-event, approved-source and freshness gated. See `docs/PROVIDER_DATA_POLICY.md`.
