...
`scripts/test-manifest.mjs` owns all three lanes: every step declares its lane there once, and `test:quick` (data/route checks) and `test:units` (script unit tests) are that same ordered list filtered, so they cannot drift out of `test:mvp`. Add a step by adding a manifest line; `npm run test:lanes` checks the manifest is coherent and runs first in both split lanes. Re-run a single failing step with `npm run test:units -- --only <step-id>`. Note `providers:validate` and `providers:identities:validate` are in neither — they live only in `npm run test:providers`, so run that for any provider or identity-registry change.
