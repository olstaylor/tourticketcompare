...
- **Price comparison:** API/feed snapshots retain their existing workflows. Separately, provider approval text supplied by the owner on 2026-07-13 permits TourTicketCompare to retrieve only the lowest publicly visible price from verified Ticketmaster and SeatGeek event pages when the scoped source is needed. The direct-page lane is manual-only until its supervised sample passes, is capped at one retrieval per event/provider in any rolling 24 hours, stores only the lowest price, currency, exact source URL and retrieval time, and never stores page content. All lanes remain feature-flag, exact-event, approved-source and freshness gated. See `docs/PROVIDER_DATA_POLICY.md`.
