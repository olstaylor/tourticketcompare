...
Last updated: 2026-09-10 (regenerated by `npm run status:validate:write` + `npm run status:surface:write`; hand-written artist/shell counts and the per-artist table corrected 2026-09-09 for the roster batch; guide content state recorded 2026-09-10 after the editorial pass; hand-written `needs_recheck` prose recounted from source 2026-09-11, and again 2026-09-13 after the 2026-09-12/13 provider syncs).
...
- `public/data/events.json`: **1427 events** — 289 `human_verified`, 863 `machine_high_confidence`, 275 `needs_recheck`. Verified event-level provider provenance is pinned one provider per line below, each separated by a blank line. Keep both properties: git needs an unchanged line between two edits to merge them as separate hunks, so without the blank lines two lanes editing adjacent providers still conflict. This makes any two provider lanes merge cleanly, except on the shared no-resale figure at the end of the list.
  - SeatGeek 286 (338 rows carry a stored `seatgeek_url`); 127 `needs_recheck` rows retain a standalone SeatGeek CTA.

  - Vivid Seats 954; 152 `needs_recheck` rows retain a standalone Vivid Seats CTA.

  - TicketNetwork 905.

  - Ticket Liquidator 761.

  - StubHub International 566.

  - Across all lanes, 48 `needs_recheck` rows have no independently verified resale provider. Every lane recomputes this, so two lanes running the same night still conflict here and no layout can fix that — the two values genuinely disagree. It is now the only such line; regenerate it with `npm run status:validate:write` instead of hand-merging.
...
- Route surface — **generated, do not hand-edit.** `npm run status:surface:write` refreshes this line from the audit's own render of every route (robots meta as served, not as inferred), and `npm run audit:indexable-surface:check` — which runs in `test:mvp` — warns when it goes stale. Exclusion reasons, expiry horizon and the stored baseline live in `reports/indexable-surface/`.
  ...
  Generated 2026-09-20: **1642 rendered / 418 indexable**. By type (rendered/indexable): home 1/1 · index 5/5 · static 10/10 · guide 18/18 · blog-post 4/4 · blog-tag 3/3 · artist 65/58 · city 223/67 · venue 458/106 · artist-city 855/146.
  <!-- /generated:route-surface -->
...
|---|---|---|---|---|---|---|---|
| beyonce | 2026-09-21 | 0 | 0 | 0 | 0 | — | No event records; artist-level CTA only. |
| harry-styles | 2026-04-30 | 74 | 31 | 31 | **8** | Together, Together | 21 dates added 2026-09-10 from the newly announced run (6 on-sale Australian, 15 pre-on-sale `announced`). All 21 carry blank `tour_name` pending human confirmation from the event pages. The 2 recheck rows are the Madrid short-form `ticketmaster.es` URLs. |
...
| ariana-grande | 2026-09-21 | 41 | 17 | 5 | 0 | The Eternal Sunshine Tour | 3 Sunrise rows are owner-verified "page loads, not on sale via TM" and render plain "Check Ticketmaster" links. |
| bad-bunny | 2026-09-21 | 28 | 0 | 0 | **4** | DeBÍ TiRAR MáS FOToS World Tour | No SeatGeek URLs (EU legs not listed on SeatGeek). Recheck rows: Marseille 7/1 and the re-added Brussels `.com` row — both CTA-suppressed. |
| morgan-wallen | 2026-09-21 | 18 | 14 | 4 | 0 | Still the Problem Tour | — |
...
| raye | 2026-09-21 | 0 | 0 | 0 | 0 | — | No event records; artist-level CTA only. |
| charli-xcx | 2026-06-18 | 17 | 10 | 10 | **1** | Music, Fashion, Film Tour | — |
| tate-mcrae | 2026-09-21 | 0 | 0 | 0 | 0 | — | No event records; artist-level CTA only. |
...
| rosalia | 2026-09-21 | 6 | 1 | 0 | 0 | LUX TOUR 2026 | Houston 6/23 renders a plain "Check Ticketmaster" link (owner-verified). |
| post-malone | 2026-09-21 | 5 | 0 | 0 | 0 | — | `tour_name` blank pending human verification. Vivid Seats covers 3 events. |
...
| jelly-roll | 2026-09-21 | 1 | 0 | 0 | **1** | — | `tour_name` blank pending human verification. |
...
| the-weeknd | 2026-09-21 | 0 | 0 | 0 | 0 | — | Promoted 2026-09-08 after verified provider checks. No event records yet, so the board renders its empty state and no CTA. |
...
| blue-october | 2026-08-28 | 66 | 17 | 17 | **24** | — | Promoted 2026-08-26; owner browser-checked both destinations. No event records yet; artist-level CTA only. |
...
| stella-lefty | 2026-09-09 | 36 | 7 | 7 | **8** | — | Promoted 2026-09-09 after owner confirmation of both API-captured provider destinations (workflow run 34371114685). Verified registry and artist links; no event records yet, so the board remains an empty watchlist without buttons. Event ingestion follows merge and deployment. |
| tobymac | 2026-09-09 | 35 | 6 | 6 | **6** | — | Promoted 2026-09-09 after owner confirmation of both API-captured provider destinations (workflow run 34371114685). Verified registry and artist links; no event records yet, so the board remains an empty watchlist without buttons. Event ingestion follows merge and deployment. |
...
| andrea-bocelli | 2026-09-09 | 25 | 2 | 2 | **3** | — | Promoted 2026-09-09 after owner confirmation of both API-captured provider destinations (workflow run 34371114685). Verified registry and artist links; no event records yet, so the board remains an empty watchlist without buttons. Event ingestion follows merge and deployment. |
