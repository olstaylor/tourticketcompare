...
- `public/data/events.json`: **1287 events** — 289 `human_verified`, 745 `machine_high_confidence`, 253 `needs_recheck`. Verified event-level provider provenance: SeatGeek 237 (289 rows carry a stored `seatgeek_url`), Vivid Seats 716, TicketNetwork 666, Ticket Liquidator 716, StubHub International 535. Of the 253 `needs_recheck` rows: 74 retain a standalone SeatGeek CTA, 90 retain a standalone Vivid Seats CTA, and 57 have no independently verified resale provider and remain fully CTA-suppressed. Of those 57, 7 are past and **50 are upcoming** — those fifty upcoming dates render no ticket CTA at all (recounted 2026-09-10 against the same `verification_status`/`provider_links.verified` test `status:validate` uses; was 16 of 23 on 2026-09-02 and 6 on 2026-08-24). The jump came with the 2026-09-10 Ticketmaster run that added 251 dates across 16 artists: ingestion lands dates far faster than resale provenance is verified for them, so this is the indexable-surface's growing edge rather than decay. Only the counts before this sentence are machine-pinned; the past/upcoming split is not, so recount it whenever an ingestion run or the calendar moves it. See BACKLOG item 4 for the verification path.
