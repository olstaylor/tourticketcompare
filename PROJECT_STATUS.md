...
- `public/data/events.json`: **1022 events** — 289 `human_verified`, 582 `machine_high_confidence`, 151 `needs_recheck`. Verified event-level provider provenance: SeatGeek 242 (294 rows carry a stored `seatgeek_url`), Vivid Seats 719, TicketNetwork 670, Ticket Liquidator 562, StubHub International 498. Of the 151 `needs_recheck` rows: 75 retain a standalone SeatGeek CTA, 88 retain a standalone Vivid Seats CTA, and 22 have no independently verified resale provider and remain fully CTA-suppressed. Of those 22, 7 are past and **15 are upcoming** — those fifteen upcoming dates render no ticket CTA at all (recounted 2026-09-02, up from 6 on 2026-08-24; only the counts before this sentence are machine-pinned, so this split moves with the calendar). Nine of the fifteen sit on artists promoted in the 2026-08-26 batch (blue-october, tyla ×2, nothing-but-thieves, sabaton ×2, beartooth ×2), so the new roster is landing dates faster than resale provenance is being verified for them.
