...
| **olivia-rodrigo** | indexable_with_substantial_content | `["ticketmaster"]` | yes | Artist-level Ticketmaster verification restored in PR #190 (issue #171 closed). 8 short-form event URLs human-checked 2026-06-10: all 404 in a browser — they remain `needs_recheck` with CTAs suppressed. `tour_name` populated ("The Unraveled Tour") on all 86 events. |
...
Olivia Rodrigo's artist-level Ticketmaster verification was restored in PR #190 (issue #171 closed). The eight short-form event URLs were human-checked in a browser on 2026-06-10 and all return 404; they remain `needs_recheck` with CTAs suppressed (the safe state) until working storefront URLs exist. `tour_name` is populated on all 86 events (issue #172).
...
- **Blank `tour_name` (#172) — resolved for all current artists.** Olivia Rodrigo (86, "The Unraveled Tour"), Bruno Mars (56, "The Romantic Tour"), and Shakira (30, "Las Mujeres Ya No Lloran"; owner-confirmed from the Ticketmaster event page 2026-06-10) are populated. The validator's blank-`tour_name` warning is clear. Issue #172 can be closed once verified on `main`.
- **Onboarding drift (#175) — CLOSED/delivered.** `validate-artist.mjs` (`npm run artist:check`) covers `artists.json`, `catalog.json`, `events.json`, partition files, `VERIFIED_TICKET_LINKS` (out.js), and the shows.js affiliate map. As of 2026-06-11 both former drift sources are derived at runtime (shows.js from out.js, signup allowlist from artists.json), eliminating those drift classes. `docs/ADDING_ARTISTS.md` and `docs/SAFE_NEXT_ARTIST_WORKFLOW.md` are the canonical onboarding docs. Retained here only as a pointer; no further work outstanding.
