...
Last updated: 2026-09-02 (regenerated by `npm run status:validate:write` + `npm run status:surface:write`; hand-written facts corrected 2026-09-02).
...
- `content/blog/*.md`: **0 published blog posts, 4 drafts, 0 tags** — all three previously published posts were withdrawn to drafts because their claims no longer matched the code and policy they described. Source of truth for the blog; `public/data/blog-content.json` is generated from it and must never be hand-edited. Recount with `npm run blog:build`.
...
- Route surface — **generated, do not hand-edit.** `npm run status:surface:write` refreshes this line from the audit's own render of every route (robots meta as served, not as inferred), and `npm run audit:indexable-surface:check` — which runs in `test:mvp` — warns when it goes stale. Exclusion reasons, expiry horizon and the stored baseline live in `reports/indexable-surface/`.
  ...
  Generated 2026-09-03: **1127 rendered / 309 indexable**. By type (rendered/indexable): home 1/1 · index 5/4 · static 9/9 · guide 18/18 · blog-post 0/0 · blog-tag 0/0 · artist 50/42 · city 173/56 · venue 299/73 · artist-city 572/106.
  <!-- /generated:route-surface -->
