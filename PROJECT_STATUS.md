...
Last updated: 2026-09-10 (regenerated by `npm run status:validate:write` + `npm run status:surface:write`; hand-written artist/shell counts and the per-artist table corrected 2026-09-09 for the roster batch; guide content state recorded 2026-09-10 after the editorial pass; hand-written `needs_recheck` prose recounted from source 2026-09-11).
...
Event CTAs publish independently per provider (`providerEventPublishable`; see `docs/ARCHITECTURE.md`). Across the 261 recheck rows, 120 publish SeatGeek, 140 publish Vivid Seats, 214 have at least one independently verified resale provider, and 4 (all past events) are fully CTA-suppressed. (Recounted 2026-09-11 from `public/data/events.json` through `scripts/lib/event-link-coverage.mjs`; the previous 255/114/90/203 predated the 2026-09-10 ingestion and CTA-sync runs.)
