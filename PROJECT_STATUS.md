...
Verified by direct inspection of `public/data/` and `functions/api/out.js` on 2026-07-02:

- `public/data/artists.json`: **16 records — all 16 `indexable_with_substantial_content`**, all with `verified_providers: ["ticketmaster","seatgeek"]` (5 of the 16 carry 0 events and render artist-level CTAs only).
...
- `public/data/events.json`: **400 events**; **234 carry stored SeatGeek event URLs**; 0 carry `vividseats_url` yet.
...
All 16 artists are `indexable_with_substantial_content` with `verified_providers: ["ticketmaster","seatgeek"]` and both `<slug>:ticketmaster` and `<slug>:seatgeek` entries in `VERIFIED_TICKET_LINKS`. (Event counts in the table below were last refreshed 2026-06-17 and have drifted — recount from `events.json` before relying on them; the totals above are current.) Event counts from `events.json`:
...
Note on `needs_recheck` rendering (updated 2026-07-02): event CTAs render independently per provider. The SeatGeek CTA is primary ("View tickets on SeatGeek"), the plain Ticketmaster link secondary ("Check Ticketmaster"), and either renders standalone. A `needs_recheck` event suppresses both CTAs **unless** the SeatGeek link carries its own verified provenance (`provider_links.seatgeek.verified === true` — currently zero events, so all 22+ recheck events still show "No verified ticket link is available for this date", the safe state). The shared rule is `providerEventPublishable` in `functions/api/out.js`, `functions/[[path]].js`, `public/app.js`, and `functions/api/shows.js`.
...
- **Short-form `ticketmaster.com/event/<id>` URLs remain broken across three artists — 2 Ed Sheeran, 8 Olivia Rodrigo, 12 Shakira events CTA-suppressed (22 total `needs_recheck`).** Owner short-form recheck 2026-06-16: Ed Sheeran **Glendale** confirmed working in a browser and restored as `human_verified` (short-form URL passes the runtime gate and the publishable redirect invariant; `human_verified` is exempt from the `machine_high_confidence` canonical long-form contract). Ed Sheeran **Nashville** and **Arlington**, the **Olivia Rodrigo 8**, and the **Shakira 12** were all re-confirmed 404 and stay `needs_recheck` with URLs cleared. The Discovery API still resolves the same IDs (status `onsale`) despite the browser 404s. Suppression is the correct safe state. Re-check periodically for working storefront URLs; do not restore from the Discovery `url` field alone. **Upgrade path for Glendale:** if the owner supplies the canonical long-form storefront URL the short-form redirects to, it can be promoted to `machine_high_confidence`.
- **Blank `tour_name` (#172) — resolved.** Olivia Rodrigo (86, "The Unraveled Tour"), Bruno Mars (56, "The Romantic Tour"), Shakira (30, "Las Mujeres Ya No Lloran"; owner-confirmed 2026-06-10), Ed Sheeran (27, "The Loop Tour"; owner-confirmed 2026-06-12), and Summer Walker (10, "Still Finally Over It"; owner-confirmed 2026-06-16) are all populated. No outstanding blank-`tour_name` events remain; the validator's blank-`tour_name` warning no longer fires.
