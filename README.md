...
- Never perform unapproved provider scraping. The only direct-page exception is the provider-authorized, catalog-scoped Ticketmaster/SeatGeek lowest-price snapshot workflow documented in `docs/PROVIDER_DATA_POLICY.md`; it stores no page content and enforces a 24-hour interval per canonical provider event ID.
