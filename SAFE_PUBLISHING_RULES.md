...
- Agents must not invent data, scrape providers, auto-publish artists, or create new governance docs. Auto-publishing events is allowed only via the three sanctioned automation paths above (the validated new-show auto-merge, the nightly lossless field-sync, and the nightly SeatGeek CTA sync) — never ad hoc. Edit the canonical docs (`CLAUDE.md` → `PROJECT_STATUS.md` → `BACKLOG.md`) instead of adding parallel ones.
