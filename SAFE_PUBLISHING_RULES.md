...
- **Generated-content auto-commit (owner-approved 2026-08-01).** `.github/workflows/content-build.yml` may auto-commit **one generated file**, `public/data/blog-content.json`, recompiled from `content/blog/*.md`, and only after `test:mvp`, `schema:validate`, the indexable-surface check and `git diff --check` pass in-job on exactly that output. It publishes no data records: event, artist, catalog and provider files are untouched, and the Markdown it compiles is human-authored. This is a build step, not a discovery path — it is separate from the five event auto-publish exceptions and does not extend them.
