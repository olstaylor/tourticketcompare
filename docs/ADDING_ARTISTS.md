...
Generated by `npm run artist:promote -- --slug nova-skye --url <verified-url>` (dry-run by default; `--write` applies). The shape it produces:
...
`functions/api/shows.js` and `functions/api/signup.js` need no per-artist snippets — both are derived at runtime.
