...
Field-level template and validation checklist for adding artists. The preferred roster-growth path is the API-captured batch workflow; this document also covers the single-artist fallback.
...
**Before starting:** Complete the proposal and human-verification gates in `docs/SAFE_NEXT_ARTIST_WORKFLOW.md`. That document defines the Proposal → Shell → Promote → Events phases, the gate checklists between them, and the AI proposal prompt. The field-level templates and example format below apply once a proposal has cleared Gate 1. For multiple artists, use `npm run artists:onboard:propose` → `npm run artists:promote:batch`; do not hand-copy identity URLs.
...
The signup allowlist is loaded from `public/data/artists.json` at runtime. Every artist record — including `review_required` ones — accepts watchlist signups as soon as the `artists.json` entry exists (Shell phase). If the allowlist cannot be loaded, `/api/signup` fails closed with `artist_validation_unavailable` (503) for artist-tagged signups.
...
- [ ] Rights and commercial/link model confirmed for every provider being added (plain Ticketmaster or approved affiliate provider)
...
- [ ] `VERIFIED_TICKET_LINKS` entry added to `functions/api/out.js` (Promote phase only — destination URL live, hostname allowlisted; `npm run artist:promote` generates this edit)
- [ ] `functions/api/shows.js` and `functions/api/signup.js` NOT hand-edited (both are derived at runtime — from `out.js` and `artists.json` respectively)
