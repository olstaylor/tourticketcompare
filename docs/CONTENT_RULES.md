...
- **Page `lastmod`** is maintained by `scripts/sync-content-provenance.mjs`, which fingerprints each static route's copy and advances the date only when that fingerprint changes. Do not hand-edit it: for a trust route edit the copy in `functions/_route-metadata.js`, for a guide edit `content/guides/<slug>.md`, then run `npm run content:provenance`. A guide's date is not written into its Markdown at all — it is held in `data/content-provenance.json` and emitted into `functions/_guide-routes.generated.js`, so automation never rewrites a file an editor is also editing. `content:provenance:check` runs in `test:mvp` and fails a commit whose published dates disagree with its copy.
- **`datePublished`** is set once, when a page first goes live, and never moves. For guides this is enforced rather than trusted: the first publication is recorded in `data/content-provenance.json` under `guide_publication`, and `npm run guides:build` fails if a guide's `date_published` ever disagrees with that record. The entry is append-only, so it survives the guide being drafted, renamed or deleted — which is also how the build knows a withdrawn guide needs a redirect.
- **A source's `last_checked` is editorial and human-only.** It means a person re-read the cited source and confirmed the guide still describes it correctly. No automation may set it.
- **A source's `linkCheckedAt` is automated and narrower.** It means only that the cited URL still resolved, and is recorded by `scripts/check-guide-source-links.mjs` in the daily audit — in `data/guide-source-link-checks.json`, never in the Markdown, so there is structurally nothing for automation to overwrite in an author's file. It renders as "link checked" precisely so it cannot be read as an editorial review. A 401/403/429 is recorded as blocked and stamps nothing — a WAF challenge is not a successful check.
