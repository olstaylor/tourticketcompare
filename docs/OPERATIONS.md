...
| `automation-health.yml` | completed scheduled run of any watched lane, every 6 hours (:17) + dispatch | Read-only outside observer for the nine scheduled lanes. Reports a lane whose recent scheduled runs failed, and a lane GitHub has stopped invoking — neither of which a workflow can detect about itself. Updates the rolling `automation:health` issue and closes it when every lane is healthy. Writes nothing to the repository and never reruns, dispatches, or merges. |
...
- **Red `main` stopped four scheduled lanes (resolved 2026-09-11).** PR #935 was merged while its head's Prelaunch Validation run was red — `content:provenance:check` was failing because the `/about` and `/about/ollie-taylor` copy it added had not been re-fingerprinted. `main` stayed red from 10:37Z to 13:29Z, and every lane that gates on `test:mvp` failed against that tree: three Impact marketplace provider-sync runs and one Vivid Seats CTA sync published nothing. Nothing reported it — the lanes write their rolling issues only on success — so it surfaced only when a human read the Actions tab, hours in. Fixed by regenerating the provenance state (#939). Two follow-ups: `automation-health.yml` now reports the same situation within minutes, and **`Prelaunch Validation` is still not a required status check on `main`**, so the merge that started this remains possible today. Making it required is a repository-settings change and needs an owner decision.
- **Self-referential `node_modules` symlink tracked on `main` (open, verified 2026-09-11).** `git ls-tree main` carries `node_modules` as a mode-120000 symlink pointing at its own absolute path, committed through #928. `.gitignore` listed `node_modules/` with a trailing slash, which matches directories but not symlinks, which is how it passed. CI is unaffected because `npm ci` removes the path before installing, but every fresh clone gets an `ELOOP` in place of its dependencies, so local and agent validation cannot run until it is cleared by hand. The fix is in PR #940, which is currently conflicted against `main`.
