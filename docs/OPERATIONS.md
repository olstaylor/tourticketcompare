...
| `GITHUB_OAUTH_CLIENT_ID` / `GITHUB_OAUTH_CLIENT_SECRET` | `/admin` editor OAuth handshake | Not configured yet; sign-in 503s naming what's missing until these and the `admin.tourticketcompare.com` custom domain exist. |
...
- **`/admin` content editor unreachable (open, re-verified 2026-08-04).** `admin.tourticketcompare.com` doesn't resolve in DNS and the apex correctly 404s `/admin` — intended fail-closed state, not a regression. Owner actions, in order: add the custom domain to the Pages project, create the GitHub OAuth App with callback `https://admin.tourticketcompare.com/api/admin/callback`, add `GITHUB_OAUTH_CLIENT_ID`/`GITHUB_OAUTH_CLIENT_SECRET` as encrypted Pages secrets (steps: `docs/BLOG.md`). Editing `content/blog/*.md` directly in a text editor or GitHub's web editor is unaffected and ships through the same pipeline.
