...
4. The public lanes default on; an explicit provider `*_PUBLIC_ENABLED=false` remains the emergency kill switch. TicketNetwork and StubHub International price display default on behind exact-ID/source/freshness/cache gates. Ticket Liquidator price display defaults off while its feed lacks numeric `CurrentPrice`. **Rights to display its pricing were owner-confirmed on 2026-09-12** ("show pricing where available"), so rights are no longer the blocker; the feed is. Enabling it needs, in this order: a read-only dry run of `impact-marketplace-price-snapshots.yml` (`provider: ticket-liquidator`, `apply: false`) showing a non-zero `usable` count, then the provider added to that workflow's scheduled matrix, then a scheduled apply run that actually writes rows, and only then `TICKETLIQUIDATOR_PRICE_DISPLAY_ENABLED=true`. Step 2 is one change, not two: the workflow's scheduled matrix and the `snapshotsScheduled` flag for this lane in `scripts/check-price-snapshot-freshness.mjs` must move together, because that script's self-test reads the workflow and asserts the two agree. Flipping the display flag first shows nothing and alarms nothing — the `/api/shows` gate is fail-closed on a cached row, and the freshness probe treats a lane as expected only when the flag is on **and** `snapshotsScheduled` is true, so the lane stays correctly "not expected" until a writer exists. Adding it to the scheduled matrix first, before the dry run, is the shortcut that actually breaks something: it turns that hourly lane red on every tick, because its automated-write gate fails the run when `usable` or `written` is zero.
...
6. **History:** approved for archival and historical display when the provider/source attribution and observation time remain attached — the same grant SeatGeek and Vivid Seats carry above. **Owner-confirmed 2026-09-21** for **TicketNetwork and StubHub International**, recorded here at the point of confirmation because this section previously documented CTAs, listed price and JSON-LD redistribution but never historical display, while `/api/price-history` had been serving these lanes' history since Phase 1. Ticket Liquidator is excluded for the same reason it is excluded everywhere else: no numeric price lane, display off. Attribution is not decorative — a displayed historical figure must name the provider that recorded it and the instant it was observed, which is why `functions/_event-price-low.js` returns those fields and cannot express an anonymous minimum.
...
- **Where a price may be rendered.** A gated snapshot may appear outside a CTA badge only when it is the same snapshot that badge would print on the same render, attributed to its own event, provider, currency and capture time. Four render sites satisfy this today: the per-show CTA buttons (`renderProviderCtaButtonHtml`); the per-date price answer at the top of an artist-city page (`renderArtistCityPriceAnswer`); the by-city link list on an artist page (`artistCityNextDatePriceLabel`), which names one city's next tracked date, that date's own lowest listed snapshot, the provider offering it and its capture time; and the per-date tables on an artist price guide (`renderPriceGuideCityTables`, `/artists/<artist>/ticket-prices`), which are the artist-city answer table repeated for every city the artist plays, rows in calendar order and never ranked. All four derive every figure from the same `serverShowCtaSpecs` output rather than re-deriving eligibility. There is no second price gate, so a lane the badge withholds can never reach the answer table or the by-city list. The capture time is not optional at any of them: `artistCityNextDatePriceLabel` drops the price from the row entirely when `fetchedAt` will not format, because an event date in a future year conveys nothing about when the figure beside it was captured.
- **A recorded low is a historical figure, not a current one.** The artist-city answer table may also show the lowest listed price recorded for **one event** over a trailing 30-day window (`functions/_event-price-low.js`). It is bounded the same way every other comparison is — same local event, same currency, providers never converted — and it is computed only for a lane that is passing the live display gate at that render, so a lane whose badge is withheld contributes nothing however cheap its history. Because `provider_pricing_history` is change-only, the window's opening state comes from the newest row at or before the window start; a naive `observed_at >= start` query would report "no history" for a price that had been standing unchanged for months. The published figure can never exceed the current price on the button beside it, because the live snapshot is itself a candidate. It always carries the provider and the observation time, per the History grants above.
- **A recorded price move is historical, per event and per provider.** The artist price guide may state the latest recorded change for one date (`functions/_event-price-moves.js`): the newest `provider_pricing_history` figure that differs from the price on that date's button, for the **same provider, same event and same currency**, with both observation times attached — the History grants above. It is computed only for a lane passing the live display gate at that render, implausible rows are floored out on read, and it is never computed across dates or providers. A count of per-date moves ("N lower and M higher than that ticket site's previous recorded figure") aggregates same-event comparisons and ranks nothing. Face value is not displayed anywhere: no approved source supplies it, and a hand-entered figure would be a manually entered price.
- **Ranking stays same-event.** The artist-city answer table identifies the lower listed snapshot *within one date* across providers, which the side-by-side rule above approves. So does the **"Lowest listed" badge** on every date card (`lowestListedProvider` in `functions/[[path]].js`, owner-requested 2026-09-24): it runs `deriveCityDatePrices` over that one card's own `serverShowCtaSpecs`, so only lanes whose button prints a price compete, mixed currencies drop the comparison, and it marks a provider only when at least two lanes are priced and one is strictly lower (a single priced lane or a tie at the bottom earns no badge). It labels an existing CTA badge rather than rendering a new figure, leaves the fixed button order unchanged, and sits beside the card's note that these are listed prices, not final totals. The answer table orders its rows by date and never ranks them against each other, and `functions/_artist-city-prices.js` deliberately exposes no cross-row minimum — so no "\<city\> tickets from \<price\>" claim can be composed from it. A minimum taken across different local events is not covered by any current grant; adding one needs the same programme-level confirmation that the schema-offers exception took, recorded here and in `SAFE_PUBLISHING_RULES.md` before any code changes.
