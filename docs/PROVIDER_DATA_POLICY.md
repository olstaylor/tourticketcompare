...
- **Where a price may be rendered.** A gated snapshot may appear outside a CTA badge only when it is the same snapshot that badge would print on the same render, attributed to its own event, provider, currency and capture time. Two render sites satisfy this today: the per-show CTA buttons (`renderProviderCtaButtonHtml`), and the per-date price answer at the top of an artist-city page (`renderArtistCityPriceAnswer`), which derives every figure from the same `serverShowCtaSpecs` output rather than re-deriving eligibility. There is no second price gate, so a lane the badge withholds can never reach the answer table.
