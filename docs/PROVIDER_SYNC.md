...
The recogniser withholds any date Ticketmaster does not report `onsale`, with one exception: an `offsale` date whose `sales.public.startDateTime` is still in the future is proposed as `announced` with that value carried verbatim in `public_onsale_at`. Until that time passes, the page and `app.js` render no ticket CTA for it, only "Public on-sale <time> per Ticketmaster." Cancelled, postponed, rescheduled, and offsale-without-a-future-on-sale dates stay withheld. `/api/out` is unchanged, so a hand-built `showId` URL for such a date still redirects to the Ticketmaster listing; only the rendered buttons are suppressed.

The recogniser also consults a tombstone registry, `data/deleted-events.json`. When an owner deletes a row from `events.json` that Ticketmaster still lists (a de-duplicated or dead-storefront copy), adding that row's ids and/or venue/date to the registry stops the next run from re-proposing it. Record the row's `events.json` `id` too: it is the event's permanent identity (price history, `/api/out` showId, future event URL key), and it is lost with the row otherwise. Matching mirrors the live dedup keys (Ticketmaster id **or** normalized venue/date), tombstoned candidates are withheld with a distinct reason, and the registry can only ever withhold more — never widen what gets proposed. A missing or malformed file fails open (no tombstones).

The nightly field-sync (`nightly-data-sync.yml`, via `scripts/apply-tm-updates.mjs`) updates only lossless factual fields on existing events — date/time, venue/city, the official listing title `event_name`, and the refreshed canonical TM URL — pulled per event id from the Discovery API when the Discovery identity is exact and that row has no review blockers of its own. It then regenerates the partitions and publishes the result through an auto-merged PR (see [DEPLOYMENT.md → Repository write capability](DEPLOYMENT.md#repository-write-capability)). Manual runs default to `dry_run: true`; a dry-run writes `.audit/tm-sync.json` as an uploaded artifact and cannot commit or push. The publish is blocked by any error, missing report, dry-run input, validation failure, smoke-test failure, or absent `events.json` diff; review items and blocked updates on other events go to the rolling `automation:data-sync` issue (via `scripts/report-tm-sync-review.mjs`) without vetoing clean updates on unrelated events. Findings tied exclusively to past events remain in the run artifact but are omitted from the current-action list; an unknown or malformed date stays actionable, and a past local row whose Ticketmaster response moves into the future stays actionable. Deletions (404/410), new shows, and `tour_name` are never auto-applied. Lifecycle status is: a Discovery `dates.status.code` of `cancelled`/`canceled`, `postponed` or `rescheduled` is stored verbatim as `ticketmaster_status_code` and removed when Ticketmaster reports the event `onsale` again. Cancelled and postponed withhold every ticket link for the date everywhere (`eventLifecycleHeld` in `functions/_route-indexability.js`) and are still listed in the issue so an owner can decide whether to remove and tombstone the row; rescheduled keeps its links, takes its new date from the same record, and is labelled rescheduled. A hold is recorded whenever the identity checks pass, even when the start time is not confirmed; recording `rescheduled`, and clearing any code, needs a record with no review blockers, so a rescheduled label never sits on a date that was not updated. `report:link-coverage` and `report-price-coverage.mjs` report held dates separately and leave them out of their zero-link and coverage gates. `offsale` and unrecognised codes are never recorded and stay review-only. The single status exception: a row carrying `public_onsale_at` flips `announced` → `on-sale` (and drops the field) when Ticketmaster reports that event `onsale`, and follows a moved public on-sale time while it stays `offsale`. Without `TICKETMASTER_API_KEY` the run writes a skipped report and no-ops safely.
...
4. Verification-date bumps via `scripts/bump-verified-dates.mjs`, published without human review after in-job validation (`npm run test:mvp` + `git diff --check`) through a branch whose PR the run squash-merges once the required `test-mvp` check has been earned on it; owner-approved 2026-07-28, replacing the former human-review PR. A TM-skip/failure guard prevents date bumps when TM data is unavailable, and the conservative blocked-link guard holds date bumps while liveness results are WAF/bot-blocked (401/403/429) — blocked is not proof of life or death.
