...
(`functions/api/shows.js` and `functions/api/signup.js` are derived at runtime and never edited per-artist.)
...
# ("indexed artist has 0 events", audit finding U2) — that is not a failure.
...
| Per-artist | `npm run artist:check -- <slug>` | Yes (WARN ok) | Yes (provider wiring PASS; 0-event WARN ok) | Yes |
| Provider drift | `validate-artist-provider-claims.mjs` | Yes | Yes | Yes |
| Provider identity registry | `providers:identities:validate` | No | Yes | If registry changed |
