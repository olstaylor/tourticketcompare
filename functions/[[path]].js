...
function renderProviderFallback(catalog, artist, surface, providerAvailability = {}) {
  ...
  return `<section class="provider-panel"><h2>Artist-level provider pages</h2><p class="muted">These are provider pages for the artist, not verified links for a specific date. Use them to check current provider listings, then confirm the final price, fees, availability, and delivery terms before buying.</p>${singleProviderNote}<div class="provider-actions">${cards}</div><p class="disclosure-note">Some links are affiliate links. This does not change your price. Final prices, fees, and availability are confirmed on the ticketing platform.</p></section>`;
}
