...
function renderShowCardServerHtml(show, seatGeekAvailable = false, isIndexableArtist = true, vividSeatsAvailable = false, artistName = "", marketplaceAvailability = {}, artistSlug = "", venueRuns = {}) {
  ...
  return `<article class="info-card show-card"${anchorId ? ` id="${escapeAttr(anchorId)}"` : ""}${show.id ? ` data-event-id="${escapeAttr(String(show.id))}"` : ""} data-show-json="${showJson}">${badgeHtml}<div class="show-card-body"><h3 class="show-card-title">${escapeHtml(title)}</h3>${subHtml}${runHtml}${ctaHtml}${copyLinkHtml}</div></article>`;
}
...
function renderShowBoardEmptyStateHtml(artistName = "", providerCta = null, artistSlug = "", pastShows = []) {
  ...
  const primaryCta = providerCta
    ? anchor(`Check ${escapeHtml(providerCta.name)} for updates`, emptyStateHref, "button button-primary", `rel="${escapeAttr(outboundCtaRel(emptyStateHref) || "noopener")}" data-cta-provider="${escapeAttr(slugify(providerCta.name))}" data-cta-artist="${escapeAttr(artistSlug)}" data-cta-price-snapshot="absent" data-cta-location="empty_state"`)
