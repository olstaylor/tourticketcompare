...
import { VENUE_IMAGES } from "./_venue-images.generated.js";
...
// One venue photograph, with the credit its licence requires.
//
// This is the only photography on the site, and it is of buildings. Artist
// photography stays absent on purpose: a free licence settles the
// photographer's copyright but not the subject's personality rights, and a
// performer's face beside an affiliate ticket button is what a false-endorsement
// claim is built from. See docs/VENUE_IMAGES.md.
//
// Not lazy-loaded, deliberately. There is exactly one image per venue page and
// it sits directly under the lead, so it is the likely LCP element — deferring
// it would delay the paint the metric measures. Width and height are always
// emitted from the real file, so the space is reserved and nothing shifts.
function renderVenuePhotoHtml(venueSlug) {
  const image = VENUE_IMAGES[String(venueSlug || "").trim()];
  if (!image?.src || !image.alt || !image.credit?.line) return "";
  const { credit } = image;
  const link = (href, label) =>
    `<a href="${escapeAttr(href)}" rel="noopener">${escapeHtml(label)}</a>`;
  return `<figure class="venue-photo"><img src="${escapeAttr(image.src)}" alt="${escapeAttr(
    image.alt
  )}" width="${escapeAttr(image.width)}" height="${escapeAttr(
    image.height
  )}" decoding="async" fetchpriority="high" /><figcaption class="disclosure-note">Photo by ${escapeHtml(
    credit.author
  )}, ${link(credit.licence_url, credit.licence)}, via ${link(credit.source_url, "Wikimedia Commons")}.</figcaption></figure>`;
}
...
export function renderVenuePageBody(route, events = [], options = {}) {
  ...
  return shell(
    ...
    )}</p>${renderVenuePhotoHtml(venue.slug)}<section class="section-grid"><div class="section-intro"><h2>Upcoming shows at ${escapeHtml(
      venue.venue
