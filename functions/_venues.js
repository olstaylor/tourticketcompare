...
const DERIVEVENUES_MEMO = new WeakMap();
export function deriveVenues(events, options = {}) {
  if (Number.isFinite(options.now) || !Array.isArray(events)) return deriveVenuesUncached(events, options);
  const minute = Math.floor(Date.now() / 60000);
  const hit = DERIVEVENUES_MEMO.get(events);
  if (hit && hit.minute === minute) return hit.result;
  const result = Object.freeze(deriveVenuesUncached(events, options));
  DERIVEVENUES_MEMO.set(events, { minute, result });
  return result;
}

function deriveVenuesUncached(events, options = {}) {
