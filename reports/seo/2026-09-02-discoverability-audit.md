...
| Search Console Core Web Vitals / CrUX | unavailable | The live report (last updated 3 September) says there is not enough real-world usage data in the last 90 days for either mobile or desktop. No field-vital conclusion is made; lab results remain diagnostic only. |
| Search Console rich-result enhancements | available | Live reports last updated 3 September show 56 valid Event items and 26 valid Breadcrumb items, with zero invalid items in either report. Event notices are optional `endDate`/`organizer` (56), optional Offer `validFrom`/`availability` (51), and five no-offer events; no schema change is warranted because availability claims are prohibited by the publishing rules. |
| Search Console HTTPS | available | The live HTTPS report last updated 4 September shows 27 HTTPS URLs, zero non-HTTPS URLs, and no detected issues in the prior 90 days. |
| Google Search Console | available | On 4 September the property selector exposed the Domain property (`sc-domain:tourticketcompare.com`) and the apex URL-prefix property (`https://tourticketcompare.com/`); no `www` or Pages-host property is accessible in this account selector. The Domain property has the apex sitemap submitted (Success; 311 discovered pages; last read 2 September). Its live Performance UI exposed 5 July–2 September totals and query, page, country and device breakouts. Query×page and coverage exports remain unavailable. The same Domain property is linked to GA4. |
