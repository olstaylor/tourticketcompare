...
export function extractTicketmasterPrice(text, event) {
  const base = { eventId: event.id, provider: "ticketmaster", sourceUrl: event.ticketmaster_url, checkedAt: new Date().toISOString(), priceType: "displayed_from", includesFees: null };
  if (BLOCKED.test(text)) return normalizeObservation({ ...base, currency: event.currency || "USD", lowestPriceMinor: null, status: "blocked" });
