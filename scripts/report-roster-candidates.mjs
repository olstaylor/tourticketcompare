...
export function renderCandidates(manifest, run = null) {
  ...
  const lines = [
    ...
    pass.length ? pass.map((r) => `- **${r.name}** (\`${r.slug}\`)${r.requested ? " · owner-requested" : ""}${stat(r)} · title: ${r.screen.seo_title}`).join("\n") : "None.",
    "",
    `### Held for a human (${held.length})`,
    held.length ? held.map((r) => `- **${r.name}** (\`${r.slug}\`)${r.requested ? " · owner-requested" : ""}: ${(r.screen?.reasons || ["not screened"]).join("; ")}`).join("\n") : "None.",
    "",
